 ...
 */
import { html, noChange, nothing, render, svg, } from '../lit-html.js';
import * as litHtmlLib from '../lit-html.js';
import { directive, Directive, PartType } from '../directive.js';
import { assert } from '@esm-bundle/chai';
import { stripExpressionComments, stripExpressionMarkers, } from './test-utils/strip-markers.js';
...
suite('lit-html', () => {
    ...
    const assertRender = (r, expected, options) => {
    ...
    suite('text', () => {
        const assertNoRenderedNodes = () => {
            ...
            assert.isEmpty(children.filter((node) => node.nodeType !== Node.COMMENT_NODE));
        };
    ...
    suite('attributes', () => {
        ...
        test('renders to an attribute expression after an attribute literal', () => {
            ...
            assert.include(['<div a="b" foo="bar"></div>', '<div foo="bar" a="b"></div>'], stripExpressionComments(container.innerHTML));
        });
        ...
        test('renders multiple bound attributes', () => {
            render(html `<div foo="${'Foo'}" bar="${'Bar'}" baz=${'Baz'}></div>`, container);
        ...
        test('renders undefined in attributes', () => {
            ...
            assert.equal(stripExpressionComments(container.innerHTML), '<div attribute="it\'s "></div>');
        });
        test('renders undefined in attributes', () => {
        ...
        test('noChange works', () => {
            ...
            go('A');
            assert.equal(stripExpressionComments(container.innerHTML), '<div foo="A"></div>', 'A');
            const observer = new MutationObserver(() => { });
            observer.observe(container, { attributes: true, subtree: true });
            go(noChange);
            assert.equal(stripExpressionComments(container.innerHTML), '<div foo="A"></div>', 'B');
        ...
        test('noChange works on one of multiple expressions', () => {
            ...
            go('A', 'B');
            assert.equal(stripExpressionComments(container.innerHTML), '<div foo="A:B"></div>', 'A');
            go(noChange, 'C');
            assert.equal(stripExpressionComments(container.innerHTML), '<div foo="A:C"></div>', 'B');
        });
    });
    ...
    suite('compiled', () => {
        ...
        const trustedTypes = globalThis
            .trustedTypes;
        const policy = (_a = trustedTypes === null || trustedTypes === void 0 ? void 0 : trustedTypes.createPolicy('lit-html', {
            createHTML: (s) => s,
        })) !== null && _a !== void 0 ? _a : { createHTML: (s) => s };
        test('only text', () => {
    ...
    suite('directives', () => {
        ...
        test('PartInfo includes metadata for directive in ChildPart', () => {
            ...
            assert.equal(stripExpressionMarkers(container.innerHTML), '<div>test</div>');
        ...
        suite('ChildPart invariants for parentNode, startNode, endNode', () => {
            ...
            const checkNodePropertiesBehavior = directive(CheckNodePropertiesBehavior);
        ...
        test('renders directives on AttributeParts', () => {
            ...
            render(go(), container);
            assert.equal(stripExpressionMarkers(container.innerHTML), '<div foo="A:1"></div>');
            render(go(), container);
            assert.equal(stripExpressionMarkers(container.innerHTML), '<div foo="A:2"></div>');
        });
        test('renders multiple directives on AttributeParts', () => {
            const go = () => html `<div foo="a:${count('A')}:b:${count('B')}"></div>`;
            render(go(), container);
            assert.equal(stripExpressionMarkers(container.innerHTML), '<div foo="a:A:1:b:B:1"></div>');
            render(go(), container);
            assert.equal(stripExpressionMarkers(container.innerHTML), '<div foo="a:A:2:b:B:2"></div>');
        });
        test('PartInfo includes metadata for directive in AttributeParts', () => {
            ...
            assert.equal(stripExpressionMarkers(container.innerHTML), '<div title="a 1 b"></div>');
        ...
        test('renders directives on PropertyParts', () => {
            ...
            assert.equal(stripExpressionMarkers(container.innerHTML), '<div></div>');
            assert.strictEqual(container.firstElementChild.foo, 'A:1');
        });
        ...
        test('directives have access to renderOptions', () => {
            ...
            class HostDirective extends Directive {
                render(v) {
                    var _a;
                    return `${(_a = this.host) === null || _a === void 0 ? void 0 : _a.value}:${v}`;
                }
                update(part, props) {
                    var _a;
                    (_a = this.host) !== null && _a !== void 0 ? _a : (this.host = part.options.host);
                    return this.render(...props);
                }
            }
            const hostDirective = directive(HostDirective);
            render(html `<div attr=${hostDirective('attr')}>${hostDirective('node')}</div>`, container, { host: hostEl });
    ...
    suite('async directives', () => {
        const aDirective = directive(class extends AsyncDirective {
            ...
            update(_part, [promise]) {
                ...
                return (_a = this.value) !== null && _a !== void 0 ? _a : this.render(promise);
            }
        });
        const bDirective = directive(class extends Directive {
            constructor() {
    ...
    securityHooksSuiteFunction('enahnced security hooks', () => {
        ...
        test('sanitizes text content when the text is alone', () => {
            ...
            assert.equal(stripExpressionMarkers(container.innerHTML), '<div>safeString</div>');
        ...
        test('sanitizes text content when the text is interpolated', () => {
            ...
            assert.equal(stripExpressionMarkers(container.innerHTML), '<div>hello safeString world</div>');
            const safeBig = new FakeSanitizedWrapper('big');
            render(getTemplate(safeBig), container);
            assert.equal(stripExpressionMarkers(container.innerHTML), '<div>hello big world</div>');
            assert.deepEqual(sanitizerCalls, [
                {
                    values: ['big', safeBig],
                    name: 'data',
        ...
        test('sanitizes full attribute values', () => {
            ...
            assert.equal(stripExpressionMarkers(container.innerHTML), '<div attrib="safeString"></div>');
            const safe = new FakeSanitizedWrapper('good');
            render(getTemplate(safe), container);
            assert.equal(stripExpressionMarkers(container.innerHTML), '<div attrib="good"></div>');
            assert.deepEqual(sanitizerCalls, [
                {
                    values: ['bad', safe],
                    name: 'attrib',
    ...
    suite('internal', () => {
        test('clearContainerForLit2MigrationOnly', () => {
            ...
            assert.equal(stripExpressionComments(container.innerHTML), INTERNAL ? '<p>HELLO</p>' : '<div>TEST</div><p>HELLO</p>');
        });
    });
});
//# sourceMappingURL=lit-html_test.js.map