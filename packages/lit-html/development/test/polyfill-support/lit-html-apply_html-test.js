...
suite('@apply', () => {
    ...
    test('styles with css custom properties using @apply render in different contexts', async () => {
      ...
      `;
        ...
        const testApplyUser = () => {
            ...
            (_a = extraGlobals.ShadyCSS) === null || _a === void 0 ? void 0 : _a.styleElement(applyUser);
            const applyUserDiv = applyUser.shadowRoot.querySelector('div');
            const applyUserStyle = getComputedStyle(applyUserDiv);
            assert.equal(applyUserStyle.getPropertyValue('border-top-width').trim(), '2px');
    ...
    `;
            ...
            assert.ok(div === null || div === void 0 ? void 0 : div.hasAttribute('some-attr'));
            assert.ok(div === null || div === void 0 ? void 0 : div.textContent, 'test');
            (_a = extraGlobals.ShadyCSS) === null || _a === void 0 ? void 0 : _a.styleElement(applyProducer);
            const usersInProducer = applyProducer.shadowRoot.querySelectorAll('apply-user');
            renderShadowRoot(applyUserContent, usersInProducer[0]);
            (_b = extraGlobals.ShadyCSS) === null || _b === void 0 ? void 0 : _b.styleElement(usersInProducer[0]);
            renderShadowRoot(applyUserContent, usersInProducer[1]);
            (_c = extraGlobals.ShadyCSS) === null || _c === void 0 ? void 0 : _c.styleElement(usersInProducer[1]);
            const userInProducerStyle1 = getComputedStyle(usersInProducer[0].shadowRoot.querySelector('div'));
            const userInProducerStyle2 = getComputedStyle(usersInProducer[1].shadowRoot.querySelector('div'));
            assert.equal(userInProducerStyle1.getPropertyValue('border-top-width').trim(), '10px');
            assert.equal(userInProducerStyle1.getPropertyValue('padding-top').trim(), '20px');
            assert.equal(userInProducerStyle2.getPropertyValue('border-top-width').trim(), '10px');
            assert.equal(userInProducerStyle2.getPropertyValue('padding-top').trim(), '20px');
    ...
    test('@apply styles flow to custom elements that render in connectedCallback', () => {
        class E extends HTMLElement {
            ...
            <div>Testing...</div>`;
                ...
                (_a = extraGlobals.ShadyCSS) === null || _a === void 0 ? void 0 : _a.styleElement(this);
            }
        }
        ...
        const userInProducerStyle1 = getComputedStyle(user1.shadowRoot.querySelector('div'));
        const user2 = applyProducer.shadowRoot.querySelector('apply-user-ce2');
        const userInProducerStyle2 = getComputedStyle(user2.shadowRoot.querySelector('div'));
        assert.equal(userInProducerStyle1.getPropertyValue('border-top-width').trim(), '10px');
        assert.equal(userInProducerStyle1.getPropertyValue('padding-top').trim(), '20px');
        assert.equal(userInProducerStyle2.getPropertyValue('border-top-width').trim(), '5px');
