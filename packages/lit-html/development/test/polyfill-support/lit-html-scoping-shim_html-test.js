...
suite('ShadyCSS scoping shim', () => {
    ...
    test('scoped styles are applied for non-TemplateResult values', function () {
        ...
        extraGlobals.ShadyCSS.ScopingShim.prepareAdoptedCssText([':host { border-top: 2px solid black; }'], 'scope-1');
        document.body.appendChild(container);
        renderShadowRoot(undefined, container);
        assert.equal(getComputedStyle(container).getPropertyValue('border-top-width').trim(), '2px');
    ...
    test('adopted CSS remains when rendering a TemplateResult after an initial non-TemplateResult', function () {
        ...
        extraGlobals.ShadyCSS.ScopingShim.prepareAdoptedCssText([':host { border-top: 2px solid black; } button { font-size: 7px; } '], 'scope-2');
        document.body.appendChild(container);
        renderShadowRoot(undefined, container);
        assert.equal(getComputedStyle(container).getPropertyValue('border-top-width').trim(), '2px');
        renderShadowRoot(html `<button>This is a button.</button>`, container);
        assert.equal(getComputedStyle(container).getPropertyValue('border-top-width').trim(), '2px');
        assert.equal(getComputedStyle(container.shadowRoot.querySelector('button'))
    ...
    test.skip('Styles inserted in the initial render through ChildParts are scoped.', function () {
        ...
        document.body.appendChild(container);
        renderShadowRoot(html `${style}<button>This is a button.</button>`, container);
        assert.equal(getComputedStyle(container).getPropertyValue('border-top-width').trim(), '2px');
        assert.equal(getComputedStyle(container.shadowRoot.querySelector('button'))
            .getPropertyValue('font-size')
            .trim(), '7px');
...
//# sourceMappingURL=lit-html-scoping-shim_html-test.js.map