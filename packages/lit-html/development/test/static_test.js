...
suite('static', () => {
    ...
    test('static attribute binding', () => {
        ...
        assert.equal(stripExpressionComments(container.innerHTML), '<div class="cool"></div>');
    ...
    test('static attribute name binding', () => {
        ...
        assert.equal(stripExpressionComments(container.innerHTML), '<div foo="bar"></div>');
        render(html `<div x-${literal `foo`}="${'bar'}"></div>`, container);
        assert.equal(stripExpressionComments(container.innerHTML), '<div x-foo="bar"></div>');
    });
    test('static attribute name binding', () => {
    ...
    test('dynamic binding after static text binding', () => {
        ...
        assert.equal(stripExpressionComments(container.innerHTML), '<p>Hello</p>&lt;p&gt;World&lt;/p&gt;');
    ...
    test('static bindings are keyed by static values', () => {
        ...
        const t = (tag, text) => html `<${unsafeStatic(tag)}>${text}</${unsafeStatic(tag)}>`;
        ...
        assert.equal(stripExpressionComments(container.innerHTML), '<div>def</div>');
        ...
        assert.equal(stripExpressionComments(container.innerHTML), '<span>abc</span>');
        const span = container.querySelector('span');
        assert.isNotNull(span);
        render(t('span', 'def'), container);
        assert.equal(stripExpressionComments(container.innerHTML), '<span>def</span>');
    ...
    test('interpolating statics into statics', () => {
        ...
        assert.equal(stripExpressionComments(container.innerHTML), '<div>a<span>b</span>c</div>');
    });
