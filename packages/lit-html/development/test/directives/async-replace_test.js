...
suite('asyncReplace', () => {
    ...
    test('replaces content as the async iterable yields new values (AttributePart)', async () => {
        ...
        assert.equal(stripExpressionMarkers(container.innerHTML), '<div></div>');
        await iterable.push('foo');
        assert.equal(stripExpressionMarkers(container.innerHTML), '<div class="foo"></div>');
        await iterable.push('bar');
        assert.equal(stripExpressionMarkers(container.innerHTML), '<div class="bar"></div>');
    });
    ...
    test('replaces content as the async iterable yields new values (BooleanAttributePart)', async () => {
        ...
        assert.equal(stripExpressionMarkers(container.innerHTML), '<div hidden=""></div>');
