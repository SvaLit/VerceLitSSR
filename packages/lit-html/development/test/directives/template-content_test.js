...
suite('templateContent', () => {
    ...
    test('renders a template', () => {
        ...
        assert.equal(stripExpressionMarkers(container.innerHTML), '<div><div>aaa</div></div>');
    });
    test('clones a template only once', () => {
        const go = () => render(html `<div>${templateContent(template)}</div>`, container);
        go();
        assert.equal(stripExpressionMarkers(container.innerHTML), '<div><div>aaa</div></div>');
    ...
    test('renders a new template over a previous one', () => {
        const go = (t) => render(html `<div>${templateContent(t)}</div>`, container);
        go(template);
        assert.equal(stripExpressionMarkers(container.innerHTML), '<div><div>aaa</div></div>');
        const newTemplate = document.createElement('template');
        newTemplate.innerHTML = '<span>bbb</span>';
        go(newTemplate);
        assert.equal(stripExpressionMarkers(container.innerHTML), '<div><span>bbb</span></div>');
    });
...
//# sourceMappingURL=template-content_test.js.map