...
suite('until directive', () => {
    ...
    test('renders non-Promises immediately', async () => {
        ...
        render(html `<div>${until(deferred.promise, defaultContent)}</div>`, container);
        assert.equal(stripExpressionMarkers(container.innerHTML), '<div><span>loading...</span></div>');
        deferred.resolve('foo');
        await deferred.promise;
        assert.equal(stripExpressionMarkers(container.innerHTML), '<div>foo</div>');
    });
    test('renders primitive low-priority content only once', async () => {
        const go = () => render(html `<div>${until(deferred.promise, 'loading...')}</div>`, container);
        go();
        assert.equal(stripExpressionMarkers(container.innerHTML), '<div>loading...</div>');
        deferred.resolve('foo');
        await deferred.promise;
        assert.equal(stripExpressionMarkers(container.innerHTML), '<div>foo</div>');
        go();
    ...
    test('renders non-primitive low-priority content only once', async () => {
        const go = () => render(html `<div>${until(deferred.promise, html `loading...`)}</div>`, container);
        go();
        assert.equal(stripExpressionMarkers(container.innerHTML), '<div>loading...</div>');
        deferred.resolve('foo');
        await deferred.promise;
    ...
    test('renders a Promise to an attribute', async () => {
        ...
        assert.equal(stripExpressionMarkers(container.innerHTML), '<div></div>');
        await promise;
        assert.equal(stripExpressionMarkers(container.innerHTML), '<div test="foo"></div>');
    });
    test('renders defaultContent to an attribute', async () => {
        const promise = Promise.resolve('foo');
        render(html `<div test=${until(promise, 'bar')}></div>`, container);
        assert.equal(stripExpressionMarkers(container.innerHTML), '<div test="bar"></div>');
        await promise;
        assert.equal(stripExpressionMarkers(container.innerHTML), '<div test="foo"></div>');
    });
    test('renders a Promise to an interpolated attribute', async () => {
        ...
        assert.equal(stripExpressionMarkers(container.innerHTML), '<div></div>');
        await promise;
        assert.equal(stripExpressionMarkers(container.innerHTML), '<div test="value:foo"></div>');
    });
    test('renders defaultContent to an interpolated attribute', async () => {
        const promise = Promise.resolve('foo');
        render(html `<div test="value:${until(promise, 'bar')}"></div>`, container);
        assert.equal(stripExpressionMarkers(container.innerHTML), '<div test="value:bar"></div>');
        await promise;
        assert.equal(stripExpressionMarkers(container.innerHTML), '<div test="value:foo"></div>');
    });
    test('renders a Promise to a property', async () => {
        ...
        assert.equal(stripExpressionMarkers(container.innerHTML), '<div></div>');
        assert.equal(div.test, undefined);
        await promise;
        assert.equal(stripExpressionMarkers(container.innerHTML), '<div></div>');
        assert.equal(div.test, 'foo');
    });
    ...
    test('renders racing Promises across renders correctly', async () => {
        ...
        const deferred2 = new Deferred();
        const t = (promise) => html `<div>${until(promise)}</div>`;
    ...
    test('renders Promises with changing priorities', async () => {
        ...
        const t = (p1, p2) => html `<div>${until(p1, p2)}</div>`;
    ...
    test('renders low-priority content when arguments change', async () => {
        ...
        assert.equal(stripExpressionMarkers(container.innerHTML), '<div>string</div>');
        // Await a microtask to let both Promise then callbacks go
        await 0;
        assert.equal(stripExpressionMarkers(container.innerHTML), '<div>string</div>');
    ...
    test('renders a literal in an AttributePart', () => {
        ...
        assert.equal(stripExpressionMarkers(container.innerHTML), '<div data-attr="a"></div>');
    });
    test('renders literals in an interpolated AttributePart', () => {
        render(html `<div data-attr="other ${until('a')} ${until('b')}"></div>`, container);
        assert.equal(stripExpressionMarkers(container.innerHTML), '<div data-attr="other a b"></div>');
    });
    test('renders a literal in a BooleanAttributePart', () => {
        render(html `<div ?data-attr="${until('a')}"></div>`, container);
        assert.equal(stripExpressionMarkers(container.innerHTML), '<div data-attr=""></div>');
    });
    test('renders a literal in an EventPart', () => {
        let callCount = 0;
        render(html `<div @some-event="${until(() => callCount++)}"></div>`, container);
        const div = container.querySelector('div');
    ...
    test('renders a literal in a PropertyPart', () => {
        ...
        assert.equal(container.querySelector('div').someProp, 'a');
    });
    ...
    test('renders a Promise in a AttributePart', async () => {
        render(html `<div data-attr="${until(Promise.resolve('a'))}"></div>`, container);
        assert.equal(stripExpressionMarkers(container.innerHTML), '<div></div>');
        await laterTask();
        assert.equal(stripExpressionMarkers(container.innerHTML), '<div data-attr="a"></div>');
    });
    test('renders Promises in an interpolated AttributePart', async () => {
        render(html `<div data-attr="other ${until(Promise.resolve('a'))} ${until(Promise.resolve('b'))}"></div>`, container);
        assert.equal(stripExpressionMarkers(container.innerHTML), '<div></div>');
        await laterTask();
        assert.equal(stripExpressionMarkers(container.innerHTML), '<div data-attr="other a b"></div>');
    });
    ...
    test('renders a Promise in a PropertyPart', async () => {
        render(html `<div .someProp="${until(Promise.resolve('a'))}"></div>`, container);
        assert.equal(container.querySelector('div').someProp, undefined);
        await laterTask();
        assert.equal(container.querySelector('div').someProp, 'a');
    });
    test('renders a Promise in an EventPart', async () => {
        let callCount = 0;
        render(html `<div @some-event="${until(Promise.resolve(() => callCount++))}"></div>`, container);
        const div = container.querySelector('div');
        div.dispatchEvent(new Event('some-event'));
    ...
    test('renders promise-likes in an interpolated AttributePart', async () => {
        ...
        render(html `<div data-attr="other ${until(thenableA)} ${until(thenableB)}"></div>`, container);
        assert.equal(stripExpressionMarkers(container.innerHTML), '<div></div>');
        await laterTask();
        assert.equal(stripExpressionMarkers(container.innerHTML), '<div data-attr="other a b"></div>');
    });
    ...
    test('promises later in the arguments array than the currently used value ' +
        'do not overwrite the current value when they resolve', async () => {
        let resolvePromiseA;
    ...
    test('promises earlier in the arguments array than the currently used ' +
        'value overwrite the current value when they resolve', async () => {
        let resolvePromiseA;
        const promiseA = new Promise((resolve, _reject) => {
            resolvePromiseA = resolve;
        });
    ...
    test('promises later in the arguments array than a non-promise are never ' +
        'rendered', async () => {
        let resolvePromise;
        const promise = new Promise((resolve, _reject) => {
            resolvePromise = resolve;
...
//# sourceMappingURL=until_test.js.map