 ...
 */
const assertLightDom = (el, str, opt) => {
 ...
 *   ],
 */
const assertHTML = (container, html) => {
    if (typeof html !== 'object') {
        ...
        for (const query in html) {
            ...
            if (query === 'root') {
                ...
                assertLightDom(container, subHtml, {
...
const modes = ['vm', 'global'];
export const setupTest = async (tests, testFile, mode = 'vm') => {
    suite(`${testFile}: ${mode}`, () => {
        let container;
        let observer;
        let _mutations;
        ...
        const observeMutations = (container) => {
        ...
        for (const [testName, testDescOrFn] of Object.entries(tests)) {
            const testSetup = typeof testDescOrFn === 'function' ? testDescOrFn() : testDescOrFn;
            ...
            testFn(testName, async () => {
                ...
                const response = await fetch(`http://localhost:9090/${mode}/${testFile}/${testName}`);
                ...
                const stableNodes = stableSelectors.map((selector) => container.querySelector(selector));
                clearMutations();
                ...
                for (const { args, html, setup, check } of expectations) {
                    ...
                    const checkNodes = stableSelectors.map((selector) => container.querySelector(selector));
