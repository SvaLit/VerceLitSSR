...
export class LitElementRenderer extends ElementRenderer {
    ...
    *renderShadow(renderInfo) {
        const serverControllers = getControllers(this.element)
            ?.map((c) => c.serverUpdateComplete)
            .filter((p) => !!p);
        if (serverControllers?.length > 0) {
            const continuation = Promise.all(serverControllers).then((_) => this._renderShadowContents(renderInfo));
            yield continuation;
        }
        else {
    ...
    *_renderShadowContents(renderInfo) {
        // Render styles.
